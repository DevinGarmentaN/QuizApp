...
export const TopNavbar: React.FC<TopNavbarProps> = ({ onOpenAiModal, onOpenNewQuizModal }) => {
  ...
  return (
    <header className="bg-slate-900 text-white shadow-md border-b border-slate-800 sticky top-0 z-40">
                ...
                <div className="absolute right-0 mt-1.5 w-76 bg-white text-slate-800 rounded-2xl shadow-2xl border border-slate-200 py-2 z-30 divide-y divide-slate-100 animate-in fade-in zoom-in-95">
                  ...
                  <div className="p-1.5 space-y-1">
                    <button
                      ...
                      className="w-full text-left px-3 py-2 text-xs font-semibold text-slate-700 hover:bg-slate-100 rounded-xl flex items-center space-x-2 transition cursor-pointer"
                    >
        ...
        <div className="fixed inset-0 z-50 overflow-y-auto bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl border border-slate-200 w-full max-w-md overflow-hidden animate-in fade-in zoom-in-95 text-slate-800">
            <div className="p-6">
              ...
              <p className="text-xs text-slate-500 leading-relaxed">
                Tindakan ini akan membersihkan seluruh cache lokal browser (LocalStorage, SessionStorage, dan cache client). Data di Cloud Firestore tetap aman.
              </p>
              ...
              <div className="mt-6 flex items-center justify-end space-x-2">
                ...
                <button
                  ...
                  className="px-4 py-2 text-xs font-bold text-white bg-amber-600 hover:bg-amber-700 rounded-xl shadow-xs transition cursor-pointer active:scale-95"
                >
      ...
      {/* Toast Notification */}
      {cacheClearedToast && (
        <div className="fixed bottom-6 right-6 z-50 bg-slate-900 text-white px-4 py-3 rounded-xl shadow-2xl border border-slate-700 flex items-center space-x-2 text-xs font-medium animate-in fade-in slide-in-from-bottom-4">
          <span className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse"></span>
          <span>Cache aplikasi dan browser berhasil dibersihkan!</span>
        </div>
      )}
